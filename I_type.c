...
void I_type_func(unsigned int op, unsigned int rs, unsigned int rt,short int im)
{
    ...
    case 0x23: // lw
        ...
        s[rt] = (d_mem[s[rs]+im]<<24) | (d_mem[s[rs]+im+1] << 16) | (d_mem[s[rs]+im+2] << 8) | (d_mem[s[rs]+im+3]);
        break;
    case 0x21: // lh
        s0_Overwrite(rt);
        OverFlow_add(s[rs],im,s[rs]+im);
        ...
        s[rt] = (d_mem[s[rs]+im]<<24>>16) | (d_mem[s[rs]+im+1]);
        break;
    case 0x25: // lhu
        s0_Overwrite(rt);
        OverFlow_add(s[rs],im,s[rs]+im);
        ...
        if(Misalignment((s[rs]+im)%2)) break;
        s[rt] =  (d_mem[s[rs]+im]<<8) | (d_mem[s[rs]+im+1]);
        break;
    case 0x20: // lb
        s0_Overwrite(rt);
        OverFlow_add(s[rs],im,s[rs]+im);
